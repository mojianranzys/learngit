...
def _excelAddSheet(csv_name, excelWriter, sheet_name):
    ...
    excelWriter.close()
pd.DataFrame().to_excel(os.path.join(os_chdir, 'mutscan_fusion_virus_combine.xlsx'))
excelWriter = pd.ExcelWriter(os.path.join(os_chdir, 'mutscan_fusion_virus_combine.xlsx'),engine='openpyxl')

sample = path.split("/")[-2]
print(sample)
mutscan_csv = os_chdir + sample + '_ffpe_vs_cfdna_mutscan_result.csv'
fusion_csv = os_chdir + sample + '_ffpe_vs_cfdna_fusion_result.csv'
virus_csv = os_chdir + sample + '_ffpe_vs_cfdna_virus_result.csv'
...
sExcelFile=os_chdir + 'mutscan_fusion_virus_combine.xlsx'
