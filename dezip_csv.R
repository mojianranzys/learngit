...
sampleSheet2 <- paste0("/haplox/runPipelineInfo/",  input2,  "/sequence_", input2,  ".csv")
    ...
    csv_df3 <- csv_df1[csv_df1[, 9] %in% c("cfdna", "healthcfdna", "ttdna", "ffpedna") & grepl("肠康",csv_df1[,25]), ]
#    print(csv_df3[,15])
    if(nrow(csv_df3) <= 0){
    ...
    for(i in seq(nrow(csv_df3))){
            for(j in seq(nrow(csv_df4))){
                if(csv_df3[i, 3] == csv_df4[j, 3] && csv_df3[i, 15] == csv_df4[j, 15]){
                    ...
                    print(restore_dir)
                    oss_restore <- paste0("/haplox/users/zhaoys/dezip/oss_restore_",gsub("-","",date),".sh")
                    oss_download <- paste0("/haplox/users/zhaoys/dezip/oss_download_",gsub("-","",date),".sh")
                    ...
                    csv_df4[j,1],"* oss://sz-hapseq/rawfq/","20", substr(input2, 1, 4), "/", input2,  "_clinic/","\n",step=""))
                    sink()
                }
            }
        }
out_csv <- paste0("/haplox/users/zhaoys/dezip/dezip_",gsub("-","",date),".csv")
