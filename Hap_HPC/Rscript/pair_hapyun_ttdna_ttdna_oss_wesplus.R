        ...
        for(i in seq(nrow(tumor_df))){
           for(j in seq(nrow(csv_df2))){
             if(csv_df2[j,3] == tumor_df[i,3] & grepl("ttdna|ffpedna|atdna|ntdna",csv_df2[j,9]) & grepl("癌旁组织|正常组织", csv_df2[j,26])){
                ...
                project_name <- paste0("/haplox/rawout/haplox/hapyun/201901/pair_sentieon_16core64g_no_factera_", tumor_df[i,1],"/tmp/*")
...
"###\n","python3 /haplox/users/zhaoys/Hap_HPC/pair_ttdna_wes_warning.py -d ",rawout,"/",tumor_df[i,1],"  >> ",rawout,"/","warning.txt", "\n",sep = ""))
                 sink()
            }
        }
    }
