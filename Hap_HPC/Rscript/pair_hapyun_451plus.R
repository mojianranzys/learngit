...
#    csv_df <- read.csv(input, header=FALSE, stringsAsFactors = FALSE, encoding = "UTF-8")
    ...
    tumor_df <- csv_df[csv_df[,9] %in% c("cfdna","healthcfdna","ffpedna","pedna","ttdna"),]
    print(tumor_df[,c(2,23)])
    tumor_df$normal = rep(NA, nrow(tumor_df))
    if(nrow(tumor_df) >= 1){ 
        for(i in seq(nrow(tumor_df))){
           for(j in seq(nrow(csv_df2))){
            if(csv_df2[j,3] == tumor_df[i,3] && grepl("gdna|saldna|healthdna|ntdna",csv_df2[j,9])){
                tumor_df[i,"normal"] <- csv_df2[j,1]
                out <- paste0(rawout,"/",tumor_df[i,1])
                sh_file <- paste0(out,"/last_pair_451plus.sh")
                if(!file.exists(out)){
                ...
                virus_cfdna <- paste0(rawout, "/", tumor_df[i, 1], "/virus/0001.virus_resulttxt_cfdna_output_37.txt")
                virus_gdna <- paste0(rawout, "/", tumor_df[i, 1], "/virus/0001.virus_resulttxt_gdna_output_37.txt")
cat(paste("###\n","ossutil cp -r oss://sz-hapres/haplox/hapyun/201901/pair_mutant_pe_sentieon_gencore_no_umi_",
...
"###\n","ossutil cp -r oss://sz-hapres/haplox/hapyun/201901/pair_mutant_pe_sentieon_gencore_no_umi_",
tumor_df[i, 1], "/tmp/fastp-16core64g_node_2/reports/ --include S* ", out,"/fastp-16core64g_node_2/reports/\n",
"###\n","ossutil cp -r oss://sz-hapres/haplox/hapyun/201901/pair_mutant_pe_sentieon_gencore_no_umi_",
tumor_df[i, 1], "/tmp/fastp-16core64g_node_1/reports/ --include S* ", out, "/fastp-16core64g_node_1/reports/\n",
"###\n","ossutil cp -r oss://sz-hapres/haplox/hapyun/201901/pair_mutant_pe_sentieon_gencore_no_umi_",
...
"###\n","echo ossutil cp -r oss://sz-hapres/haplox/hapyun/201901/pair_mutant_pe_sentieon_gencore_no_umi_",
...
"###\n","mv ", virus_cfdna, " ", out, "/", tumor_df[,1], "_um.bam.result.txt","\n",
"###\n","mv ", virus_gdna, " ", out , "/", tumor_df[i,"normal"], "_um.bam.result.txt","\n",
