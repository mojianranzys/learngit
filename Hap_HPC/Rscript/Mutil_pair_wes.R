    ...
    if(nrow(tumor_df) >= 1){
        for(i in seq(nrow(tumor_df))){
           for(j in seq(nrow(csv_df2))){
              if(csv_df2[j,3] == tumor_df[i,3] & grepl("atdna|ntdna|gdna",csv_df2[j,9]) & grepl("wesplus", csv_df2[j,1])){
                out <- paste0(rawout,"/",tumor_df[i,1])
                csv_df3 <- c(csv_df3,out)
                sh_file <- paste0(rawout,"/muilt_pair_ttdna_wes.sh")
                ...
                sink(sh_file,append = TRUE) #append = TRUE 不覆盖原有内容， append = FALSE(默认) ：覆盖原有内容
